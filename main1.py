...
def update_medicine_listbox():
    ...
    for med in medicine_list:
        ...
        row = tree.insert("", tk.END, values=(med["meal"], med["start"], med["end"], status, time_left))
...
def delete_selected_medicine():
    ...
    med_name = medicine_list[index]["name"]
    confirm = messagebox.askyesno("Delete", f"Are you sure you want to delete '{med_name}'?")
    if confirm:
        del medicine_list[index]
        update_medicine_listbox()
        if logged_in_user:
            save_user_medicines(logged_in_user['username'], medicine_list)
        messagebox.showinfo("Deleted", f"'{med_name}' has been removed.")
...
tree = ttk.Treeview(app_frame, columns=("Meal", "Start", "End", "Status", "Time Left"), show="headings")
...
logout_btn = tk.Button(app_frame, text="Logout", command=lambda: do_logout())
logout_btn.pack(pady=10)

# Show login and register initially
